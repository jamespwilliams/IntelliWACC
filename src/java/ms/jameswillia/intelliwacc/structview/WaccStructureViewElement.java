...
public class WaccStructureViewElement implements StructureViewTreeElement, SortableTreeElement {
	protected final PsiElement element;

	public WaccStructureViewElement(PsiElement element) {
	...
	public ItemPresentation getPresentation() {
		return new WaccItemPresentation(element);
	}

	@NotNull
	@Override
	public TreeElement[] getChildren() {
		if ( element instanceof WaccPSIFileRoot ) {
			Collection<? extends PsiElement> funcs = XPath.findAll(WaccLanguage.INSTANCE, element, "/script/function/ID");
			List<TreeElement> treeElements = new ArrayList<>(funcs.size());
			for (PsiElement el : funcs) {
				treeElements.add(new WaccStructureViewElement(el));
			}
