...
public class WaccParserDefinition implements ParserDefinition {
	public static final IFileElementType FILE =
		new IFileElementType(WaccLanguage.INSTANCE);
	...
	static {
		...
		List<TokenIElementType> tokenIElementTypes =
			PSIElementTypeFactory.getTokenIElementTypes(WaccLanguage.INSTANCE);
		ID = tokenIElementTypes.get(WaccLanguageLexer.IDENT);
	}

	public static final TokenSet COMMENTS =
		PSIElementTypeFactory.createTokenSet(
			WaccLanguage.INSTANCE,
			WaccLanguageLexer.COMMENT);

	public static final TokenSet WHITESPACE =
		PSIElementTypeFactory.createTokenSet(
			WaccLanguage.INSTANCE,
			WaccLanguageLexer.WS);

	public static final TokenSet STRING =
		PSIElementTypeFactory.createTokenSet(
			WaccLanguage.INSTANCE,
			WaccLanguageLexer.STRING);
	...
	public PsiParser createParser(final Project project) {
		final WaccLanguageParser parser = new WaccLanguageParser(null);
		return new ANTLRParserAdaptor(WaccLanguage.INSTANCE, parser) {
			@Override
			protected ParseTree parse(Parser parser, IElementType root) {
				// start rule depends on root passed in; sometimes we want to create an ID node etc...
				if ( root instanceof IFileElementType ) {
					return ((WaccLanguageParser) parser).prog();
				}
				// let's hope it's an ID as needed by "rename function"
				return ((WaccLanguageParser) parser).primary();
			}
		};
	}
	...
	public PsiFile createFile(FileViewProvider viewProvider) {
		return new WaccPSIFileRoot(viewProvider);
	}
	...
	public PsiElement createElement(ASTNode node) {
		...
		switch ( ruleElType.getRuleIndex() ) {
			case WaccLanguageParser.RULE_func :
				return new FunctionSubtree(node, elType);
			case WaccLanguageParser.RULE_declarationStat :
				return new VardefSubtree(node, elType);
			case WaccLanguageParser.RULE_param :
				return new ArgdefSubtree(node, elType);
			case WaccLanguageParser.RULE_beginStat :
				return new BlockSubtree(node);
			case WaccLanguageParser.RULE_functionCall :
